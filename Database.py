...
class Database:
    ...
    def record_basicInfo(self, sheet):
        conn, c = self._conn()
        sql = 'insert into 所有歌单基本信息表 ' + 'values(?, ?, ?, ?, ?)'
        c.execute(sql, sheet.baseInfo('tuple'))
        conn.commit()
        c.close()
        conn.close()

    def insert_data(self, sheet):
        conn, c = self._conn()
        sql = 'insert into ' + sheet.fileName + ' values(?, ?, ?, ?)'
        for song in sheet.songList():
            c.execute(sql, song.tuple())
        conn.commit()
        c.close()
        conn.close()
        print('往 ' + sheet.fileName + '表 插入数据成功!\n')
